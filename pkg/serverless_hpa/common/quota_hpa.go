...
func (quotaController *Quota) QuotaReturn() bool {
	...
	if foundingMember == "true" && len(quotaPodQpsIncreaseOrDecrease) == 1 && quotaRemain == 0 && quotaRequire == quotaStep && quotaStep == quotaLocal {
		return true
	} else {
		if locker, err := etcd_lock.New(lib.EtcdEndpointQuota, quoaHpaOption); err != nil {
			klog.Errorf("创建锁失败：%+v", err)
			...
			//})
			//if retryErr != nil {
			//	panic(fmt.Errorf("pod qps quota scale down  failed: %v", retryErr))
			//	return false
			//
			//}
