...
func (c *Controller) syncHandlerParent(key string) error {
	...
	if err != nil {
		...
		if errors.IsNotFound(err) {
			c.parentQuotaDelete(key, namespace, name)
			return nil
		}
		...
		if err == nil {
			//更新操作
			c.parentQuotaUpdate(quotaparent, quotalocal, namespace, name)

		} else {
			//新建操作
			c.parentQuotaNew(namespace, name)
		}
...
func (c *Controller) parentQuotaUpdate(quotaparent *v1.Quota, quotalocal *v1.Quota, namespace string, name string) error {
	...
	//TODO 锁粒度细化，满足条件需要变更的情况下上锁并进行相关业务，否则不加锁
	if quotaLocalCopy.Spec.ClusterAreaType == "cluster" {

		//场景1: 触发0-1扩容
		//场景2: 触发quota申请情况
		//klog.Info(quotaLocalCopy)
		quotaLocalCopy.UID = ""
		...
		err := c.parentQuotaUpdateAlertZero(quotaLocalCopy, quotaLocalCopyOfParent)
		if err != nil {
			klog.Errorf(err.Error())
			return err
		}
	...
	} else if quotaLocalCopy.Spec.ClusterAreaType == "field" {
...
func (c *Controller) parentQuotaNew(name string, namespace string) error {
	...
	klog.Infof("Parent Quota对象被新建，这里执行实际的新建业务: %s/%s ...:Nothing to do", namespace, name)
	return nil

}

// TODO 本级deploy大于0，则直接删除field级的childalert相关信息并返回
				...
				retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
					...
					if *result.Spec.Replicas > 0 {
						...
						if locker, err := etcd_lock.New(etcdEndpointQuota, option); err != nil {
							...
							if quotaLocalCopy.Labels["quota.cluster.pml.com.cn/type"] != "noLimit" {
								//场景1: 触发0-1扩容
								if remain >= step {
...
func (c *Controller) zeroAlertDeploy(quotaLocalCopy *v1.Quota) error {
	...
	retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		if quotaLocalCopy.Labels["quota.cluster.pml.com.cn/type"] == "qps" {
			...
			for _, podInfo := range (*podList).Items {
				...
				if in == false {
					...
					var action = "init"
					c.qpsQuotaHpaAction(&podInfo, quotaLocalCopy.Name, hpaWebUrl, action, int64(step), int64(step))
				}
			}
		} else if quotaLocalCopy.Labels["quota.cluster.pml.com.cn/type"] == "replica" || quotaLocalCopy.Labels["quota.cluster.pml.com.cn/type"] == "noLimit" {
			quotaLocalCopy.Labels["quota.cluster.pml.com.cn/quotaRemain"] = strconv.Itoa(remain - step)
			quotaLocalCopy.Labels["quota.cluster.pml.com.cn/quotaUsed"] = strconv.Itoa(quotaUsed + step)
			klog.Infof("quotaLocalCopy labels is ", quotaLocalCopy.Labels)
			//0-1触发完成后标志删除
...
func (c *Controller) foundingMemberPodInit(quotaLocalCopy *v1.Quota) error {
	...
	retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		if quotaLocalCopy.Labels["quota.cluster.pml.com.cn/type"] == "qps" {
			...
			quotaLocalCopy.Labels["quota.cluster.pml.com.cn/quotaRemain"] = strconv.Itoa(remain - step)
			quotaLocalCopy.Labels["quota.cluster.pml.com.cn/quotaUsed"] = strconv.Itoa(quotaUsed + step)
			quotaLocalCopy.Labels["quota.cluster.pml.com.cn/init"] = "false"
			for _, podInfo := range (*podList).Items {
				klog.Infof("pods-name=%v\n", podInfo.Name)
				klog.Infof("pods-status=%v\n", podInfo.Status.Phase)
				klog.Infof("pods-condition=%v\n", podInfo.Status.Conditions)
...
func (c *Controller) scaleUpDeploy(quotaLocalCopy *v1.Quota) error {
	...
	retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		*result.Spec.Replicas = *result.Spec.Replicas + 1
		_, updateErr = deploymentsClient.Update(context.TODO(), result, metav1.UpdateOptions{})
		if updateErr != nil {
			klog.Info(updateErr)
		}
		if quotaLocalCopy.Labels["quota.cluster.pml.com.cn/type"] == "qps" {
			...
			quotaLocalCopy.Labels["quota.cluster.pml.com.cn/quotaRemain"] = strconv.Itoa(remain - step)
			quotaLocalCopy.Labels["quota.cluster.pml.com.cn/quotaUsed"] = strconv.Itoa(quotaUsed + step)
			quotaLocalCopy.Labels["quota.cluster.pml.com.cn/deployScale"] = "false"
...
func (c *Controller) ClusterStateUpdate(quotaLocalCopy *v1.Quota) error {
	...
	if oldState != quotaLocalCopy.Spec.ChildClusterState[0].ClusterState {
		retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
			// Retrieve the latest version of Deployment before attempting update
			// RetryOnConflict uses exponential backoff to avoid exhausting the apiserver
			if quotaLocalCopy.Labels["quota.cluster.pml.com.cn/type"] == "replica" || quotaLocalCopy.Labels["quota.cluster.pml.com.cn/type"] == "qps" {
				initQuotaLocalCopy, _ := c.quotaclientset.ServerlessV1().Quotas(quotaLocalCopy.Namespace).Get(context.TODO(), quotaLocalCopy.Name, metav1.GetOptions{})
				quotaLocalCopy.ResourceVersion = initQuotaLocalCopy.ResourceVersion
				_, err := c.quotaclientset.ServerlessV1().Quotas(quotaLocalCopy.Namespace).Update(context.TODO(), quotaLocalCopy, metav1.UpdateOptions{})
				if err != nil {
...
func (c *Controller) podQpsQuotaAdd(i int, quotaLocalCopy *v1.Quota) error {
	...
	quotaLocalCopy.Spec.PodQpsQuota[i].QpsQuota = quotaLocalCopy.Spec.PodQpsQuota[i].QpsQuota + step
	...
	var action = "update"
	c.qpsQuotaHpaAction(podInfo, quotaLocalCopy.Name, hpaWebUrl, action, int64(qpsQuotaOfPod), int64(step))
